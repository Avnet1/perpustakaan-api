...
class OrganizeAccessModuleService
{
    ...
    public function findAccessModule(string $id): LaravelResponseInterface
    {
        try {
            $result = $this->repository->findById($id);

            if (!$result) {
                return new LaravelResponseContract(false, 400, __('validation.custom.error.default.notFound', ['attribute' => 'ID Access Module']), $result);
            }


            return new LaravelResponseContract(true, 200, __('validation.custom.success.organizeModule.fetchModul'), $result);
        } catch (Exception $e) {
            DB::rollBack();
            return sendErrorResponse($e);
        }
